...
def run_search(dict_file, postings_file, queries_file, results_file):
    ...
    with open(queries_file) as query_file:
        ...
        query_string = content[0]
        for i in range(1, len(content)):
            relevant_docs.append(content[i].strip())
    ...
    with gzip.open(postings_file, "rb") as post_file:
        ...
        for word in re.split(reg_exp, query_string):
            ...
            if not all(letter in string.punctuation for letter in word):
                ...
                if word not already been seen before in query, and store in word_data
                """
    ...
    """
    heap = [(-value, key) for key, value in scores.items() if value != 0]
    ...
    with open(results_file, 'w') as output_file:
        ...
        for value in heapq.nsmallest(len(heap), heap):
